...
[ Pressione D4 ]  [ Pressione D5 ]
       │           │
       ▼           ▼
   • Pula sobre   • Abaixa a cabeça
     Cactos       • Evita Pássaros
   • Evita          no ar
     obstáculos
     no chão
```
...
O projeto foi desenvolvido no ecossistema PlatformIO / VS Code em linguagem C++, utilizando a arquitetura de estados e loop contínuo da plataforma Arduino.
...
```cpp
int dinoTop = isDucking ? (dinoY + 8) : dinoY;
// Reduz a área de impacto se estiver abaixado
```
...
O cabo de sinal (D4 ou D5) deve ser conectado em um lado do canal central e o cabo de GND no lado oposto.

### 2. Resistores Internos

O código ativa o modo `INPUT_PULLUP`.
...
Ao pressionar o botão e fechar o circuito com o GND:

```text
GPIO → LOW (0)
```

Dessa forma, não é necessário utilizar resistores externos.
...
```bash
git clone https://github.com/davisouzza/DinoChrome-Game.git
```
...
Aguarde o PlatformIO carregar as dependências definidas no `platformio.ini`.
...
Clique no ícone de **Upload (→)** na barra inferior do PlatformIO para compilar e gravar o código no microcontrolador.

Após o upload, o jogo estará pronto para ser executado no display OLED.

---

## 📜 Licença

Este projeto foi desenvolvido para fins educacionais e de aprendizado em sistemas embarcados e programação C++/ESP32.

Sinta-se livre para clonar, estudar e modificar o projeto.
